...
<div class="max-w-7xl mx-auto px-4">
    <div id="map" class="w-full h-[400px] rounded-lg mb-6"></div>
        ...
        <form method="GET" class="flex gap-2 mb-4">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari fasilitas..." class="border p-2 rounded w-1/3">
...
<div class="flex gap-2 mb-3 flex-wrap">
    ...
    <form method="GET" action="/export-history" class="flex gap-2">
        <input type="month" name="bulan_export" class="border p-2 rounded">
        <button
            type="submit"
        ...
        <div class="bg-white shadow rounded-lg">
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-t hover:bg-gray-50 @if($item->status == 'down') bg-red-100 @elseif($item->status == 'maintenance') bg-yellow-100 @endif">
                        <td class="p-3"> {{ $loop->iteration + ($fasilitas->currentPage() - 1) * $fasilitas->perPage() }}</td>
                        ...
                        <!-- STATUS -->
                         <td class="p-3"><span class="px-2 py-1 rounded block text-center text-white @if($item->status=='ready') bg-green-500 @elseif($item->status=='maintenance') bg-yellow-500 @else bg-red-500 @endif "> {{ strtoupper($item->status) }} </span> </td>
                        ...
                        <!-- Waktu -->
                        <td class="p-3"><div> {{ $item->updated_at->timezone('Asia/Jakarta')->format('d M Y H:i') }}</div></td>

                        <!-- FOTO -->
                        <td class="p-3"> @if($item->foto) <a href="{{ asset('storage/'.$item->foto) }}" target="_blank"> <img src="{{ asset('storage/'.$item->foto) }}" width="80" class="rounded hover:scale-110 transition"> </a> 
                        @else
                        -
                        @endif 
                        </td>
...
<!-- MODAL HISTORI -->
<div id="historyModal" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50 z-[9999]">
    <div class="bg-white rounded-lg shadow-lg w-[500px] max-h-[400px] overflow-auto p-4">
        ...
        <a id="downloadHistoryPdf" href="#" target="_blank" class="bg-red-500 text-white px-3 py-1 rounded text-sm inline-block mb-3">
         Download PDF
        </a>
...
<div id="updateModal" class="fixed inset-0 hidden items-center justify-center bg-black bg-opacity-50 z-50">
    <div class="bg-white rounded-lg shadow-lg w-[500px] p-5">
        ...
        <form id="updateForm" enctype="multipart/form-data"> <input type="hidden" id="fasilitas_id">
            <!-- STATUS -->
            <div class="mb-3">
                <label>Status</label>
            ...
            <div class="mb-3">
                ...
                <textarea id="keterangan" class="w-full border rounded p-2" rows="3"></textarea>
            </div>
            ...
            <div id="foto-container"> 
                <input type="file" name="foto[]" class="w-full border rounded p-2 mb-2"> </div>
            <div class="flex gap-2 justify-end">
                <button type="button" onclick="tambahFoto()" class="bg-green-500 text-white px-3 py-1 rounded">
                 + Tambah Foto
                </button>
                <button type="button" onclick="closeUpdateModal()" class="bg-gray-500 text-white px-4 py-2 rounded">
                    Batal
                </button>
...
<div class="mt-6 flex justify-center">
    ...
    <!-- PAGINATION -->
  <div id="pagination-wrapper" class="mt-6 flex justify-center">
    {{ $fasilitas->onEachSide(1)->links('pagination::tailwind') }}
</div>
...
function loadTable() {
    ...
    newDoc.querySelector("#pagination-wrapper").innerHTML;
    });
}
    ...
    html += `
        <tr class="border-t">
            <td class="border p-2 text-xs">
                ${item.previous_update?new Date(item.previous_update).toLocaleString():'-'}
            </td>
            <td class="border p-2 text-xs">
            ...
            <a href="/storage/${photo.foto}"target="_blank">
            <img src="/storage/${photo.foto}" width="60" class="rounded mb-1">
            </a>
        `).join('')
        :
        '-'
...
content.innerHTML = html;
    } catch(e){content.innerHTML = 'Error load data';
    }
}
function closeModal(){
    const modal = document.getElementById('historyModal');
...
<!-- Maps -->
 <script> console.log("FASILITAS MAP:", @json($fasilitasMap));</script>
 <script>
...
<!-- Upload FOTO UPDATE -->
 <script>
function tambahFoto(){let container = document.getElementById('foto-container');
    // hitung jumlah input file
    let total =
        container.querySelectorAll('input').length;
    // maksimal 5 foto
    if(total >= 5){ alert('Maksimal 5 foto');
        return;
    }
    let input =
