...
use App\Models\AuditLog;
    ...
    {
        $data = Fasilitas::with(['photos','histories.photos'])->findOrFail($id);
        $deletedFiles = [];
        // Hapus foto utama
        if ($data->foto && Storage::disk('public')->exists($data->foto) ) { Storage::disk('public')->delete($data->foto); $deletedFiles[] = $data->foto; }

        // Hapus foto fasilitas
        foreach ($data->photos as $photo) {
            if ( $photo->foto && Storage::disk('public')->exists($photo->foto) && !in_array($photo->foto, $deletedFiles)) { Storage::disk('public')->delete($photo->foto); $deletedFiles[] = $photo->foto;} $photo->delete();
        }
...
public function updateData(Request $request, $id)
{
    ...
    // SIMPAN HISTORI SETIAP KALI ADA UPDATE
    $history = FasilitasHistory::create([
        'fasilitas_id' => $fasilitas->id,
        'user_id' => auth()->id(),
        'status_from' => $oldStatus,
    ...
    if ($request->hasFile('foto')) {
        ...
        foreach ($request->file('foto') as $index => $file) {
            ...
            // FOTO PERTAMA MENJADI FOTO UTAMA FASILITAS
            if ($index == 0) {
                $fasilitas->foto = $path;
                $fasilitas->save();
            }

            // SEMUA FOTO MASUK KE HISTORI
            HistoryPhoto::create([
                'fasilitas_history_id' => $history->id,
                'foto' => $path
            ]);
